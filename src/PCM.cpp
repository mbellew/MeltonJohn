...
#endif
    w  = (float *)malloc(FFT_LENGTH*sizeof(float));
    memset(w, 0, FFT_LENGTH*sizeof(float));
...
void PCM::getPCM(float *data, CHANNEL channel, size_t samples, float smoothing)
{
    ...
    // copy
    float freq[FFT_LENGTH*2];
    float *from = channel==0 ? freqL : freqR;
...
void PCM::_updateFFT(size_t channel)
{
    ...
    float *freq = channel==0 ? freqL : freqR;
